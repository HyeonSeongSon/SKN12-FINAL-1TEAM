...
### 주요 특징
- **멀티 에이전트 아키텍처**: Router Agent가 4개의 전문화된 AI 에이전트(employee/client/docs/search)로 작업을 분배해 협업
...
### Client Agent API
`client_agent_api.py` 자체가 `APIRouter Agent(prefix="/client")`를 가지고 있어, `agent_server.py`의 `/api/v1` 프리픽스와 합쳐져 최종 경로는 `/api/v1/client/*`입니다.
```http
