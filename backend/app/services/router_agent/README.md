...
Router Agent(오케스트레이터, 실제 코드상 클래스명은 `RouterAgent`)는 사용자의 자연어 쿼리를 분석하여 적절한 전문 에이전트로 라우팅하는 지능형 멀티 에이전트 시스템입니다. LangChain과 LangGraph를 기반으로 구축되어 동적 도구 생성, 세션 관리, 인터럽트 처리 등의 고급 기능을 제공합니다.
