...
### Router Agent + 4개의 전문 AI 에이전트

#### Router Agent
- **위치**: `backend/app/services/router_agent/`
- **역할**: 사용자 요청 분석 및 작업 라우팅. 아래 4개 전문 에이전트 중 하나(또는 여러 개)로 작업을 분배하며, 그 자체는 전문 에이전트로 집계하지 않음
- **기능**:
  ```python
  # 작업 분해 예시
